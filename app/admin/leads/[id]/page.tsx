...
export default async function LeadPage({
  ...
  const details = [
    ...
    ["Submitted", date(lead.created_at)],
    ["Name", lead.full_name],
    ["Email", lead.email],
    ["Phone", lead.phone],
    ["Preferred contact", label(lead.preferred_contact)],
    ["Postcode", lead.postcode],
    ["Property", label(lead.property_type)],
    ["Track", label(lead.track_type)],
    ["Quantity", lead.track_quantity],
    ["Source", lead.source],
    ...
    <>
      <header className="admin-header">
        <div>
          ...
          <p className="admin-kicker">{lead.reference_number}</p>
          <h1>{lead.full_name}</h1>
        </div>
        <div>
          <Link
            className="admin-primary"
            href={`/admin/invoices/new?lead=${lead.id}`}
          >
            Create invoice
          </Link>{" "}
